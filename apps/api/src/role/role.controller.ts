...
export class RoleController {
  ...
  @ApiParam({ name: 'workspaceId', description: 'Workspace ID' })
  async createRole(@Param('workspaceId') workspaceId: string, @Body() dto: CreateRoleDto) {
  ...
  @ApiParam({ name: 'workspaceId', description: 'Workspace ID' })
  @ApiParam({ name: 'roleId', description: 'Role ID' })
  async deleteRole(@Param('workspaceId') workspaceId: string, @Param('roleId') roleId: string) {
  ...
  @Get('permissions/all')
  @RequirePermission({ resource: 'roles', action: 'read', scope: 'all' })
  @ApiOperation({ summary: 'Get all available system permissions (matrix data)' })
  @ApiParam({ name: 'workspaceId', description: 'Workspace ID' })
  ...
  async updateRolePermissions(
    ...
    return this.roleService.updateRolePermissions(workspaceId, roleId, dto.permissionIds);
  }
  ...
  @RequirePermission({ resource: 'roles', action: 'read', scope: 'all' })
  @ApiOperation({ summary: 'Get custom permissions assigned to a specific user' })
  @ApiParam({ name: 'workspaceId', description: 'Workspace ID' })
  @ApiParam({ name: 'userId', description: 'User ID' })
  async getUserPermissions(@Param('workspaceId') workspaceId: string, @Param('userId') userId: string) {
    return this.roleService.getUserPermissions(workspaceId, userId);
  }

  @Get('users/:userId/permissions/effective')
  @RequirePermission({ resource: 'roles', action: 'read', scope: 'all' })
  @ApiOperation({ summary: 'Get effective permissions (Role + Custom) for a specific user' })
  @ApiParam({ name: 'workspaceId', description: 'Workspace ID' })
  @ApiParam({ name: 'userId', description: 'User ID' })
  async getEffectivePermissions(@Param('workspaceId') workspaceId: string, @Param('userId') userId: string) {
  ...
  async updateUserPermissions(
    ...
    return this.roleService.updateUserPermissions(workspaceId, targetUserId, user.userId, dto.permissionIds);
  }
}
