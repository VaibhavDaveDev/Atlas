...
export class RoleService {
  ...
  async deleteRole(workspaceId: string, roleId: string) {
    ...
    if (memberCount > 0) {
       throw new BadRequestException(`Cannot delete role with active members. Please reassign ${memberCount} members first.`);
    }
  ...
  async updateRolePermissions(workspaceId: string, roleId: string, permissionIds: string[]) {
  ...
  async getEffectiveUserPermissions(workspaceId: string, userId: string) {
    ...
    const userSpecificPermissions = customPermissions.map(up => up.permission);
  ...
  async updateUserPermissions(workspaceId: string, targetUserId: string, grantedById: string, permissionIds: string[]) {
    ...
    return this.prisma.$transaction(async (tx) => {
      ...
      return { success: true, message: 'User custom permissions updated successfully' };
    });
  }
}
