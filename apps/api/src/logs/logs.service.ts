...
export class LogsService {
  ...
  constructor(
    ...
    this.lokiEnabled =
      this.configService.get<string>("LOKI_ENABLED") === "true";
  }
  ...
  async getSystemLogs(limit = 100) {
    if (!this.lokiEnabled) {
      return {
        logs: [],
        message:
          "Loki is not enabled. Please check your LOKI_ENABLED environment variable.",
      };
    }
    ...
    try {
      ...
      const response = await fetch(
        `${this.lokiUrl}/loki/api/v1/query_range?query=${query}&limit=${limit}`,
      );

      if (!response.ok) {
        throw new Error(
          `Loki returned ${response.status}: ${await response.text()}`,
        );
      }
