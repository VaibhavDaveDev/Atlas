...
export class InvoicesController {
  ...
  @Post()
  @RequirePermission({ resource: 'finance_invoices', action: 'create', scope: 'all' })
  @ApiOperation({ summary: 'Create a new invoice' })
  async create(@Body() createDto: CreateInvoiceDto, @Req() req: Request): Promise<any> {
  ...
  @Patch(':id/post')
  @RequirePermission({ resource: 'finance_invoices', action: 'update', scope: 'all' })
  @ApiOperation({ summary: 'Post an invoice to General Ledger' })
  async postInvoice(@Param('id') id: string, @Req() req: Request): Promise<any> {
    const workspaceId = (req as any).user.workspaceId;
    const userId = (req as any).user.id;
  ...
  @Get()
  @RequirePermission({ resource: 'finance_invoices', action: 'read', scope: 'all' })
