...
export class ReportsService {
  ...
  async getBalanceSheet(workspaceId: string): Promise<any> {
    ...
    return {
      assets: assets.map(a => ({ name: a.accountName, code: a.accountNumber, balance: a.balance })),
      liabilities: liabilities.map(a => ({ name: a.accountName, code: a.accountNumber, balance: a.balance })),
      equity: equity.map(a => ({ name: a.accountName, code: a.accountNumber, balance: a.balance })),
      totals: {
        assets: totalAssets,
  ...
  async getProfitAndLoss(workspaceId: string, startDate: string, endDate: string): Promise<any> {
    ...
    const report: Record<string, { name: string, type: AccountType, amount: number }> = {};

    for (const line of lines) {
      const accId = line.account.id;
      if (!report[accId]) {
        report[accId] = { name: line.account.accountName, type: line.account.accountType, amount: 0 };
      }
    ...
    const income = Object.values(report).filter(r => r.type === AccountType.INCOME);
    const expenses = Object.values(report).filter(r => r.type === AccountType.EXPENSE);
  ...
  async getDashboardStats(workspaceId: string): Promise<any> {
    ...
    const [totalAccounts, totalRevenue, totalExpenses, overdueInvoices, recentJournals, treasuryAccounts] = await Promise.all([
    ...
    const rev = Number(totalRevenue._sum.credit || 0) - Number(totalRevenue._sum.debit || 0);
    const exp = Number(totalExpenses._sum.debit || 0) - Number(totalExpenses._sum.credit || 0);
    const treasury = Number(treasuryAccounts._sum.balance || 0);

    // Monthly Chart Data (Simplified for last 6 months)
    const months: { name: string; start: Date; end: Date; revenue: number; expenses: number }[] = [];
    for (let i = 5; i >= 0; i--) {
    ...
    for (const month of months) {
      ...
      month.revenue = Number(stats[0]._sum.credit || 0) - Number(stats[0]._sum.debit || 0);
      month.expenses = Number(stats[1]._sum.debit || 0) - Number(stats[1]._sum.credit || 0);
    }
