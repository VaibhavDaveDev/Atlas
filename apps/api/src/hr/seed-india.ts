...
export async function seedIndiaStatutory(
  workspaceId: string,
  prisma: PrismaClient = prismaClient,
) {
  console.log(
    `🌱 Seeding Indian Statutory masters for workspace: ${workspaceId}`,
  );
  ...
  try {
    await prisma.$transaction(async (tx) => {
      ...
      const components = [
        ...
        },
        {
          ...
          abbr: "PT",
          type: SalaryComponentType.DEDUCTION,
          componentType: "Professional Tax",
          isTaxable: false,
        },
        {
      ...
      const slabs = [
        ...
        { fromAmount: 600000, toAmount: 900000, taxRate: 0.1 },
        { fromAmount: 900000, toAmount: 1200000, taxRate: 0.15 },
        { fromAmount: 1200000, toAmount: 1500000, taxRate: 0.2 },
        { fromAmount: 1500000, toAmount: null, taxRate: 0.3 },
      ];
    ...
    console.error(
      `❌ Error seeding India statutory for ${workspaceId}:`,
      error,
    );
    throw error;
  } finally {
