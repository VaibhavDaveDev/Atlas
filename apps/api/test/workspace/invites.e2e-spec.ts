...
describe('Workspace Invites (e2e)', () => {
  ...
  let ownerId: string;
  let testEmail = `test-invite-${crypto.randomUUID()}@example.com`;
  ...
  beforeEach(async () => {
    ...
    app.useGlobalInterceptors(new TransformInterceptor(mockWinstonLogger as any));
    await app.init();
  ...
  it('should prevent accepting a cancelled invite', async () => {
    ...
    expect(acceptRes.body.message).toContain('expired or already been processed');
  });
  ...
  it('should list pending invites and show they are gone after cancellation', async () => {
    ...
    await request(app.getHttpServer())
      .post(`/api/v1/workspaces/${workspaceId}/invites`)
      .set('Authorization', `Bearer ${workspaceToken}`)
      .send({ email: `list-test-${crypto.randomUUID()}@example.com`, role: 'USER' });
    ...
    const pendingCount = listRes.body.data.filter((i: any) => i.status === 'PENDING').length;
    expect(pendingCount).toBeGreaterThan(0);

    const inviteId = listRes.body.data.find((i: any) => i.status === 'PENDING').id;
    ...
    const finalPendingCount = listRes2.body.data.filter((i: any) => i.status === 'PENDING').length;
